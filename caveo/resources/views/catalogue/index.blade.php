...
@section('content')
    ...
    <form method="GET" action="{{ url()->current() }}">
        <div class="m-4">
            ...
            <div class="m-4">
                <button type="button" id="openFilters"
                    class="bg-[#A83248] text-white px-4 py-3 rounded w-full font-semibold">
                    Filtres
                </button>
            </div>
        </div>
        ...
        <div id="filterPanel"
            class="fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl z-50 max-h-[90vh] flex flex-col translate-y-full transition-transform duration-300">
                        ...
                        @foreach($formats as $format)
                            <label class="flex items-center gap-2">
                                <input type="checkbox" name="formats[]" value="{{ $format }}"
                                    {{ in_array($format, request('formats', [])) ? 'checked' : '' }}>
                                {{ $format }} ml
                            </label>
                        @endforeach
                    </div>
                ...
                <div>
                    ...
                    <div class="flex flex-col gap-2 max-h-40 overflow-y-auto">
                        @foreach($millesimes as $m)
                            <label class="flex items-center gap-2">
                                <input type="checkbox" name="millesimes[]" value="{{ $m }}"
                                    {{ in_array($m, request('millesimes', [])) ? 'checked' : '' }}>
                                {{ $m }}
                            </label>
                        @endforeach
                    </div>
